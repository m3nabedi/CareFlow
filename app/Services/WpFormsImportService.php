...
class WpFormsImportService
{
    ...
    public function import(array $forms, bool $replaceExisting = false): array
    {
        ...
        foreach ($forms as $form) {
            ...
            DB::transaction(function () use ($form, $replaceExisting, &$result): void {
                /** @var array<string, mixed> $settings */
                $settings = Arr::get($form, 'settings', []);
                $sourceId = (string) $form['id'];
                $questionnaire = Questionnaire::query()->where('settings->wpforms_form_id', $sourceId)->first();
    ...
    {
        return isset($field['choices']) && is_array($field['choices']) ? array_values($field['choices']) : null;
    }
